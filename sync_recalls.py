...
def merge(db, fresh):
    ...
    enf_csvs = (sorted(glob.glob(os.path.join(HERE, "fda_enforcement.jsonl")))
                or sorted(glob.glob(os.path.join(HERE, "*_enforce_rpt*.csv"))))
    if enf_csvs:
        try:
    ...
    # The FDA Enforcement Report publishes weekly, on Wednesdays. Everything
    # else here is event-driven — a Class I formula recall goes up the day FDA
    # learns of it, which is why the rest of this runs daily — but enriching
    # from a weekly source every day means ~1,900 API calls against a file that
    # moved once. Wednesday and Thursday only, or on demand.
    _weekday = datetime.datetime.utcnow().weekday()   # Mon=0
    _enrich_day = _weekday in (2, 3) or os.environ.get("FORCE_ENRICH") == "1"
    if os.environ.get("SKIP_ENRICH") != "1" and _enrich_day:
        try:
            fenf.enrich_all(db["recalls"])
        except Exception as e:
            print(f"[!] enforcement enrichment skipped: {e}", file=sys.stderr)
    elif os.environ.get("SKIP_ENRICH") != "1":
        print("[*] enforcement enrichment skipped — the FDA Enforcement Report "
              "publishes Wednesdays (FORCE_ENRICH=1 to override)")
