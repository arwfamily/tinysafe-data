...
The order below, and why each level sits where it does:
    ...
    3  contact         how it reaches the child, and therefore how fast
    4  age             whether this is a product for this child
    5  no remedy       whether anyone is coming
    6  units           whether it is likely in this house
    7  recency         whether it is likely still in this house

**Contact moved above no-remedy after a case that made the old order
indefensible.** A whole milk contaminated with Clostridium botulinum ranked
below an aftermarket crib mattress, because the mattress was a warning with no
remedy and the milk was an ordinary recall. But infant botulism arrives days
after a bottle a parent may pour tonight, and the mattress needs a baby laid on
it. Urgency is how fast the thing hurts; no-remedy is who eventually fixes it.
Those are different questions and the first one has to come first.
...
# Some families hold two kinds of product. `Bath & water safety` is mostly pool
# and spa drain covers — plumbing a child never touches — alongside baby bath
# seats a child sits in. Family alone put the drain covers in `handled` and
# ranked them above a chest of drawers, which reads wrong because it is.
_INFRASTRUCTURE = re.compile(
    r'\bdrain\s?(?:cover|port|grate)|\bsuction\s?(?:outlet|fitting)|'
    r'\bpool\s?(?:cover|fence|alarm)|\bskimmer\b|\bpump\b', re.I)
_SAT_IN = re.compile(r'\bbath\s?(?:seat|chair|ring|tub|support)|\bswim\s?(?:seat|float|vest)|'
                     r'\bfloat(?:ie|ies|s)?\b|\barm\s?bands?\b', re.I)


def contact_band(family, direct_use=True, name=''):
    """How close the product gets. Lower is closer."""
    if family == 'Bath & water safety':
        if _SAT_IN.search(name or ''):
            return CONTACT['worn']
        if _INFRASTRUCTURE.search(name or ''):
            return CONTACT['environmental']
...
def audience_band(age_band, direct_use, family=None):
    """Kept for callers that still want the pair as one number."""
...
# Each step must exceed the full range of everything below it.
#   units    0-4    -> remedy step  5
#   remedy   0-9    -> age step    10
#   age      0-39   -> contact step 50
#   contact  0-239  -> death step  250
#   deaths   0-989  -> tier step  1000
DEATH_W, CONTACT_W, AGE_W, REMEDY_W, EXPOSURE_W = 250, 50, 10, 5, 1
MAX_ADJUSTMENT = (3 * DEATH_W + 4 * CONTACT_W + 3 * AGE_W
                  + 1 * REMEDY_W + 4 * EXPOSURE_W)  # 989
...
def severity_rank(tier, deaths=0, record_type='recall', remedy_type=None,
    ...
    return ((tier or 9) * 1000
            ...
            + contact_band(family, direct_use, name) * CONTACT_W
            + AUDIENCE.get(age_band, 2) * AGE_W
...
def explain(rank):
    ...
    c, rest = divmod(rest, CONTACT_W)
    a, rest = divmod(rest, AGE_W)
    r, e = divmod(rest, REMEDY_W)
    d, c, a, r, e = min(d, 3), min(c, 4), min(a, 3), min(r, 1), min(e, 4)
    return {
        'tier': tier,
        'deaths': ['10+', '3-9', '1-2', 'none'][d],
        'remedy': ['none — dispose or warning', 'remedy exists'][r],
        'exposure': ['1M+', '100k+', '10k+', '1k+', '<1k or unstated'][e],
        'contact': ['ingested', 'mouthed', 'worn', 'handled', 'environmental'][c],
        'age': ['infant', 'toddler', 'any', 'child'][a],
    }
