...
def audience_band(age_band, direct_use):
    """0-6. Age leads; contact separates within an age."""
    return AUDIENCE.get(age_band, 2) * 2 + (0 if direct_use else 1)
...
DEATH_W, REMEDY_W, EXPOSURE_W = 200, 50, 10
MAX_ADJUSTMENT = 3 * DEATH_W + 1 * REMEDY_W + 4 * EXPOSURE_W + 6
...
def severity_rank(tier, deaths=0, record_type='recall', remedy_type=None,
                  units=None, age_band='any', direct_use=True):
    ...
    return ((tier or 9) * 1000
            ...
            + audience_band(age_band, direct_use))
...
def explain(rank):
    ...
    return {
        ...
        'audience': ['infant, direct', 'infant', 'toddler, direct', 'toddler',
                     'any, direct', 'any', 'child, direct', 'child'][min(a, 7)],
    }
