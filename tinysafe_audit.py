...
def audit_boundaries(repo):
    ...
    # An alternation is plural-tolerant if it ends in s?/es? OR if every member
    # that needs a plural already carries one inline. Flagging a stopword list
    # of prepositions ("for|at|by|to") as a plural bug is noise, and noise is
    # how a check stops being read.
    tolerant = re.compile(r'\)\s*\(\?:s\|es\)\?\\b|\)s\?\\b|s\?\||s\?\)|es\?\||es\?\)')
...
def audit_frozen(repo, records):
    ...
    for r in records:
        ...
        # Was comparing against tinysafe_audience.priority(), the legacy formula
        # that severity_rank replaced - so it reported 96% drift on a field that
        # is recomputed every merge and is correct. An audit that cries wolf on
        # its own blind spot gets ignored, which this file says elsewhere.
        if r.get('priority_rank') != tsev.severity_rank(
                r.get('tier'), r.get('deaths_reported') or 0, r.get('record_type'),
                r.get('remedy_type'), r.get('units'), r.get('age_band'),
                r.get('direct_use'), r.get('category_family'),
