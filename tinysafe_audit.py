...
def audit_frozen(repo, records):
    ...
    for r in records:
        ...
        if not ptype and 'FDA' in str(r.get('source', '')):
            ptype = {# H- is food, not drug — every H- record is formula, baby food,
        # bottled water or food colouring. Guessed wrong and it put
        # a2 Platinum infant formula into Medications.
        'Z': 'Devices', 'D': 'Drugs', 'H': 'Food',
                     'F': 'Food', 'C': 'Cosmetics'}.get(
                         str(r.get('recall_id') or '')[:1].upper(), '')
...
def audit_absence(records):
    ...
    bad = [r for r in records
           ...
           and r['priority_rank'] // 100 != r.get('tier')]
    if bad:
        fail(f"rank // 100 != tier on {len(bad)} records")
    else:
        ok("rank // 100 == tier holds")
