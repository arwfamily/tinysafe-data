...
# Deaths and injuries now lead inside a tier, so the adjustment ceiling moved:
# incidents (max 40) + age (max 30) + contact (max 5) = 75, still under 100.
...
MAX_ADJUSTMENT = 40 + max(AGE_RANK.values()) * 10 + 5
...
def priority(tier, band, direct, incident_bonus=0):
    ...
    """
    return ((tier or 9) * 100
            + incident_bonus
            + AGE_RANK.get(band, 2) * 10
            + (0 if direct else 5))
