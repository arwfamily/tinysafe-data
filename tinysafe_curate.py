...
# A product named for adults whose reported victims are adults is not a
# children's recall. "Mobility Transfer Systems Adult Portable Bed Rails" ranked
# first in the feed on three deaths — a 78-year-old woman in an assisted living
# facility among them. It passed on `bed rail`, which is there for toddler beds.
ADULT_PRODUCT = re.compile(r'\badults?\b|assisted living|nursing home|elder\w*|senior', re.I)
ADULT_VICTIM = re.compile(
    r'\b\d{2,3}[- ]year[- ]old\b|\belderly\b|assisted living|nursing home|'
    r'\b(?:man|woman|men|women)\b', re.I)


def _adult_only(product, heading, hazard, incidents=''):
    blob = f'{product} {heading}'
    if not ADULT_PRODUCT.search(blob):
        return False
    victims = f'{hazard} {incidents}'
    if not victims.strip():
        return True
    # keep it if a child is named among the victims
    return not S2_VICTIM.search(key(victims)) and bool(ADULT_VICTIM.search(victims))
...
def curate(product='', heading='', description='', hazard='', extra=''):
    ...
    if S3_STANDARD.search(all_blob):
        sig.append('standard')
    if _adult_only(product, heading, hazard, extra):
        return False, [], 'adult product, adult victims'
