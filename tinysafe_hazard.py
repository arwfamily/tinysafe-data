...
PATTERNS = {
    ...
    'device_failure': r'ventilat\w*\s+(?:fail|interrupt|stop)|deliver\w*\s+(?:incorrect|inaccurate)|malfunction|'
                      ...
                      r'fail\w*\s+to\s+(?:alarm|alert|deliver|operate)|premature (?:failure|wear)|'
                      r'\bleak\w*|crack\w*|fractur\w*|\bsplit\w*|separat\w*\s+from|detach\w*|'
                      r'may\s+(?:break|fail|crack|separate|disconnect)|does\s+not\s+(?:deliver|alarm|function)|'
                      r'incorrect\s+(?:dose|dosage|reading|display)|over-?infusion|under-?infusion',
    'unapproved':    r'510\(k\)|pre-?market\s+notification|premarket (?:clearance|approval|notification)|unapproved (?:drug|device|new drug)|without .{0,20}clearance|not cleared|misbrand|adulterat',
    'carbonmonoxide': r'carbon monoxide',

    # tier 5-8
    'asbestos':      r'asbestos',
    'subpotent':     r'subpotent|superpotent|potency|elevated\s+(?:levels?|amounts?)\s+of|'
                     r'(?:higher|lower)\s+than\s+(?:labeled|declared|expected)',
...
# Ingested mycotoxin is not mould on a fabric surface. `mold` matched patulin,
# aflatoxin and visible growth alike and gave them all tier 4, so a mycotoxin in
# infant food ranked with a mouldy tent. 21 of the 36 mould records are
# Food & formula. Same one-word-two-events shape as choking, same branch.
INGESTED_FAMILIES = {
    'Food & formula', 'Medications & supplements', 'Personal care & medicine',
}

# 16 CFR 1615/1616 exist because children burned to death in nightwear: loose
# nightgowns ignite and the child is asleep and alone. `flammable` sat flat at
# tier 3 across 234 records of which 206 are Sleepwear & apparel, ranking a
# nightgown that fails the standard alongside a flammable outdoor toy. Tier 2 -
# lethal, but the mechanism needs an ignition event - is the honest read; it is
# not tier 1, which is for hazards that need no event at all.
SLEEPWEAR_FAMILIES = {'Sleepwear & apparel', 'Sleep \u2014 mattresses & bedding'}

MOUTHABLE = {
    'Feeding & high chairs', 'Toys', 'Jewelry & accessories',
    'Personal care & medicine', 'Household chemicals', 'Button cells & batteries',
    'Oral care & teething', 'Food & formula',
}
...
def derive(hazard_text: str, product_text: str = '', family: str = None):
    ...
    for h in hits:
        ...
        if h == 'lead' and family in MOUTHABLE:
            t = 3
        # Choking is two different events wearing one word. A six-month-old
        # mouthing a teether that breaks apart has four minutes; an
        # eight-year-old swallowing a bicycle part does not. Federal law draws
        # the same line — 16 CFR 1501 applies the small-parts standard only to
        # products intended for under-threes, and the identical part is legal
        # in a toy for older children. Choking is also the leading cause of
        # toy-related death under three, at a rate comparable to furniture
        # tip-over, which sits at tier 2.
        if h == 'choking' and family in MOUTHABLE:
            t = 2
        if h == 'mold' and family in INGESTED_FAMILIES:
            t = 2
        if h == 'flammable' and family in SLEEPWEAR_FAMILIES:
            t = 2
        tiers.append(t)
...
# A record floored to tier 1 by a reported death, whose notice never used a
# hazard word, ships the generic "Serious risk" badge - so Rock 'n Play says
# less about how it kills than a teether does. The notice says "infant
# fatalities have occurred ... after the infants rolled from their back to their
# stomach", which names the mechanism without using `suffocation`.
#
# The family carries what the vocabulary does not. Only families where the
# mechanism is not in doubt are mapped; anything else keeps `general`, because
# a wrong mechanism on a death record is worse than a vague one.
DEATH_BACKFILL = {
    'Sleep \u2014 cribs, bassinets, loungers': 'suffocation',
    'Sleep \u2014 mattresses & bedding': 'suffocation',
    'Walkers, swings & bouncers': 'suffocation',
    'Bath & water safety': 'drowning',
    'Nursery furniture & tip-over': 'tipover',
}


def backfill_fatal_hazard(hazards, family):
    """Name the mechanism on a death-floored record that has no hazard word."""
    live = [h for h in (hazards or []) if h != 'general']
    if live:
        return hazards
    guess = DEATH_BACKFILL.get(family)
    return [guess] if guess else (hazards or ['general'])
