...
def flags(p):
    ...
    return {
      ...
      "_alg_coconut": any(t in b for t in ["COCOS NUCIFERA","COCONUT","COCO-CAPRYLATE","COCO-GLUCOSIDE","COCOATE","COCAMIDE","COCOYL","COCOA-"]),
      ...
      "_alg_tree_nut": bool(__import__("re").search(r"PRUNUS AMYGDALUS|\bALMOND\b|ARGAN|MACADAMIA|JUGLANS|WALNUT|CORYLUS|HAZELNUT|ANACARDIUM|CASHEW|PISTACIA|BRAZIL NUT|BERTHOLLETIA|PECAN|\bCARYA\b", b)),
      "_alg_cocoa": bool(__import__("re").search(r"THEOBROMA|\bCOCOA\b|\bCACAO\b", b)),
      "_alg_olive": bool(__import__("re").search(r"OLEA EUROPAEA|OLEA EUROPEA|\bOLIVE\b", b)),
      "_alg_castor": any(t in b for t in ["RICINUS","CASTOR"]),
      "_alg_avocado": any(t in b for t in ["PERSEA","AVOCADO"]),
      "_alg_citrus": bool(__import__("re").search(r"CITRUS|BERGAMOT|\bORANGE\b|\bLEMON\b|\bLIME\b|GRAPEFRUIT|MANDARIN|TANGERINE|AURANTIUM", b)) and not __import__("re").search(r"LEMONGRASS|CITRONELLA|CYMBOPOGON", b.replace("CITRUS","")),
      "_alg_calendula": "CALENDULA" in b,
      "_alg_safflower": any(t in b for t in ["CARTHAMUS","SAFFLOWER"]),
      "_alg_oat": bool(__import__("re").search(r"\bOAT\b|OATMEAL|OAT KERNEL|OAT FLOUR|AVENA", b)),
      ...
      "_alg_soy": (any(t in b for t in ["GLYCINE SOJA","SOYBEAN","SOJA","HYDROLYZED SOY","SOY PROTEIN","SOY ISOFLAVON","LECITHIN, SOY","SOY LECITHIN"]) or ("LECITHIN" in b and "SUNFLOWER" not in b and "SOJA" not in b) or "SOY STEROL" in b),
      "_alg_wheat": bool(__import__("re").search(r"TRITICUM|\bWHEAT\b|HORDEUM|\bBARLEY\b|SECALE|\bRYE\b|AVENA SATIVA KERNEL FLOUR", b)),
      "_alg_peanut": any(t in b for t in ["ARACHIS","PEANUT"]),
      "_alg_milk": any(t in b for t in [" MILK","CASEIN","WHEY","LACTIS PROTEIN"]),
      "_alg_egg": bool(__import__("re").search(r"\bEGG\b|ALBUMEN|\bOVUM\b|LYSOZYME|EGG PHOSPHOLIPID", b)) and "EGGPLANT" not in b,
      "_alg_sesame": any(t in b for t in ["SESAMUM","SESAME"]),
      "_alg_fish": any(t in b for t in ["FISH","COD LIVER","SALMON","MARINE COLLAGEN"]),
      "_alg_shellfish": any(t in b for t in ["SHELLFISH","CRUSTACEAN","CHITOSAN","CHITIN","GLUCOSAMINE"]),
      "_x_talc": "TALC" in b,
