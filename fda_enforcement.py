...
# openFDA returns the whole lot list, and one record came back at 2,581,627
# characters — 17 MB across 59 records, doubling the archive file on its own.
# The same cap the CSV path uses applies here.
ENRICH_CAP = 2000
...
def _cap(v):
    ...
    if len(t) <= ENRICH_CAP:
        return t
    cut = t[:ENRICH_CAP].rsplit(' ', 1)[0]
    return f"{cut} ... (+{len(t) - len(cut):,} more characters — see the official notice)"
...
def apply_enrichment(rec, src):
    ...
    for k, dest in FIELD_MAP.items():
        ...
        if not (rec.get(dest) or ""):
            rec[dest] = _cap(v) if dest in ("code_info", "more_code_info") else v
            added.append(dest)
