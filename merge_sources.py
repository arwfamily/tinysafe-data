...
def fill_legacy(rec):
    ...
    for _k in ('product_name', 'brand', 'display_name', 'heading', 'plain_reason',
               'reason', 'action', 'sold_at', 'incidents_text'):
        if rec.get(_k):
...
if __name__ == '__main__':
    ...
    app.sort(key=lambda r: (r.get('priority_rank') or 99999,
                            str(r.get('recall_date') or ''), str(r.get('recall_id') or '')))
