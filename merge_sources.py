...
if __name__ == '__main__':
    ...
    print('  by group :', dict(Counter(r.get('category_group') for r in recs).most_common()))
    ...
    LIVE_FIELDS = [
        ...
        'match_brand', 'match_text', 'match_words', 'match_squash',
        'match_brand_squash', 'match_ndc', 'category', 'action_curated',
        'curation_signals', 'grouped_products', 'product_count', 'is_enforced',
        'needs_review',
    ]
    NEW_FIELDS = [
    ...
    # Full record set, one per line: greppable, diffable per record, and a git
    # diff reads as "this field on this recall changed" instead of reflowing.
    ...
    print(f'  recalls_full.jsonl    {len(recs):5d} records  {b:5.1f} MB  (repo / reprocessing)')
