...
def _cpsc_search(recall_id):
    """CPSC's own search with the recall number pre-filled.

    Better than the bare index for the 122 archive records with no heading to
    slug: the parent lands on a page already showing their recall rather than a
    blank list they have to work out how to query.
    """
    rid = str(recall_id or '').strip()
    if not rid:
        return AGENCY_URL['CPSC']
    return f"https://www.cpsc.gov/Recalls?search_api_fulltext={urllib.parse.quote(rid)}"


def _fda_search(rec):
    """IRES search keyed on the recall number rather than the product name.

    A recall number resolves to one record; a product description matches
    whatever else shares its words. Every FDA record has one — Z- 1,360,
    F- 286, D- 284, H- 12 — so this is strictly more precise except for the
    25 press releases, which have no enforcement number and keep the name.
    """
    rid = str(rec.get('recall_id') or '').strip()
    if rid and re.match(r'^[A-Z]-\d', rid):
        return ("https://www.accessdata.fda.gov/scripts/ires/index.cfm"
                f"?Product={urllib.parse.quote(rid)}#tabNav_advancedSearch")
    name = tc.clean(rec.get('product_name') or '')[:60]
    if name:
        return ("https://www.accessdata.fda.gov/scripts/ires/index.cfm"
                f"?Product={urllib.parse.quote(name)}#tabNav_advancedSearch")
    ...
    if not rec.get("url"):
        ...
        if src == "CPSC":
            rec["url"] = ((_cpsc_url(rec["recall_id"], rec["heading"])
                          ...
                          or _cpsc_search(rec.get("recall_id")))
        elif src == "FDA":
