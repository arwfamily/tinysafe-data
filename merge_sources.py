...
AGE_SCOPE = re.compile(r"""\b(
    teen|adolescent|school.?age|junior\s?high
  | bicycles?|\bbikes?\b|tricycles?|scooters?|skateboards?|trampolines?
  | bike\s?helmets?|cycling
  | bunk\s?beds?|backpacks?|lunch\s?bo(?:x|xes)
  | craft\s?kits?|science\s?kits?
)\b""", re.X | re.I)

# Deliberately NOT in the narrowing, after looking at what it removed:
#
#   tip restraints / furniture anchors / anti-tip — these are the hardware that
#   stops furniture tip-over, which kills roughly ten children a year and is the
#   single largest environmental category in the store. A recall on the safety
#   device itself is more relevant to a 0-4 parent, not less.
#
#   "youth" as a bare word — it caught "Youth Sweatshirts with Drawstrings",
#   and drawstring strangulation is a children's hazard with its own federal
#   standard. Narrowed to teen/adolescent/school-age, which describe an age the
#   app genuinely isn't for.
#
# I restored this rule from the old pipeline without reading what it excluded.
# That is the same mistake shape as everything else caught today.
...
def fill_legacy(rec):
    ...
    if not rec.get("brand"):
        ...
        rec["brand"] = brand_from_title(rec.get("product_name")
                                        or rec.get("heading") or "")
    if not rec.get("brand"):
...
def enrich(rec):
    ...
    _in_window = (_d >= '2016') if _d.isdigit() else True
    _age_ok = AGE_SCOPE.search(tc.key(
        f"{rec.get('product_name') or ''} {rec.get('heading') or ''}")) is None
    rec['in_feed_scope'] = _in_window and _age_ok
