...
# Kids II renamed itself Kids2 in 2019 - one company, two spellings, and the
# spelling split both halves of the relation key: brand ('kidsii' vs 'kids2')
# and product ('kidsiirockingsleeper' vs 'kids2rockingsleeper'). So the 2019
# Rocking Sleeper recall (19-112, five infant deaths) and its 2023
# reannouncement (23-089, fifteen) never grouped, even though 23-089's own
# units field says "The original recall was announced in April 2019". A
# declared rename is not loose matching - only names the company itself has
# used are listed, which is what keeps the seahorse/Century class of false
# positive out.
_NAME_ALIASES = [('kidsii', 'kids2')]


def _fold_aliases(s):
    for old, new in _NAME_ALIASES:
        s = s.replace(old, new)
    return s


def _rel_key(rec):
    b = _fold_aliases(re.sub(r'[^a-z0-9]', '', str(rec.get('brand') or '').lower()))[:12]
    p = re.sub(r'\ball models? of\b|\breannounce\w*\b|\bexpand\w*\b|\brecall\w*\b',
               '', str(rec.get('display_name') or '').lower())
    p = _fold_aliases(re.sub(r'(?:es|s)$', '', re.sub(r'[^a-z0-9]', '', p)))[:22]
    return b, p
...
def fill_legacy(rec):
    ...
    for _b in ([rec.get("brand")]
        ...
        if len(_k) > 1 and _k not in _seen:
            _seen.add(_k)
            _brands.append(_b)
    # One company, two spellings: Kids II renamed itself Kids2 in 2019. The
    # 2019 Rocking Sleeper recall says Kids II, its 2023 reannouncement says
    # Kids2, and a parent registers whichever name is on their box - verified
    # in the app: registering Kids2 surfaced one record and hid the 2019
    # five-death sibling. Each record carries both spellings so either
    # registration matches. Declared renames only; this is not fuzzy matching.
    for _old, _new in (('Kids II', 'Kids2'),):
        _ko = re.sub(r'[^a-z0-9]', '', _old.lower())
        _kn = re.sub(r'[^a-z0-9]', '', _new.lower())
        if any(k == _ko or k.startswith(_ko) for k in _seen) and _kn not in _seen:
            _seen.add(_kn)
            _brands.append(_new)
